...
export default abstract class IDBC {
  ...
  public select(tableName: string, field?: IDBValidKey | IDBKeyRange, where?: any, limit?: number) {
    return this.transaction<any[]>(tableName, 'readonly', table =>
      (!field || field === table.keyPath) && table.getAll(where, limit) || table.index(field!.toString()).getAll(where, limit)
    );
  }

  public count(tableName: string, field?: IDBValidKey | IDBKeyRange, where?: any) {
    return this.transaction(tableName, 'readonly', table =>
      (!field || field === table.keyPath) && table.count(where) || table.index(field!.toString()).count(where)
    );
  }
}
